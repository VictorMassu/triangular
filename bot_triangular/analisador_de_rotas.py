...
def simular_rotas(exchange, volume_minimo=1000):
    ...
    for m1, m2 in itertools.permutations(moedas, 2):
        ...
        try:
            ...
            if preco1 < VALOR_MINIMO_MOEDA:
                raise ValueError(f"[MOEDA BAIXA] Preço muito baixo em {par1}: {preco1}")
            ...
            final = aplicar_taxa(m2_recebido * preco3 if tipo3 == "sell" else m2_recebido / preco3)
            ...
            log_info(f"🔎 Resultado: {rota} | Final = {final:.6f} | Lucro = {lucro_real:.6f} ({lucro_pct:.6f}%)")
            if lucro_pct > LUCRO_IRREALISTA:
                log_info(f"🚨 Lucro irreal detectado: {lucro_pct:.2f}% — Pode indicar erro lógico")
